...
ifeq ($(shell $(grep) -c '^modulesymfile[[:space:]]*:\?=' \
  ...
  ifneq ($(shell $(grep) -c '^output-symdump[[:space:]]*:\?=' \
                 $(KSRC)/scripts/Makefile.modpost),0)
   ifeq ($(KBUILD_EXTMOD),)
     modulesymfile := modulesymfile := modules-only.symvers
     kernelsymfile := kernelsymfile := vmlinux.symvers
   else
     modulesymfile := modulesymfile := $(KBUILD_EXTMOD)/Module.symvers
     kernelsymfile := kernelsymfile := Module.symvers $(KBUILD_EXTRA_SYMBOLS)
   endif
  endif
endif
