		...
		size_t count, loff_t *pos, bool write)
{
	...
	req->ep_addr = (u64)*pos;
	req->count = count;
	req->timeout_ms = 0; //no timeput 10 * 1000;	/* 10 seconds */
				...
				size_t count, loff_t *pos)
{
	...
	if ((u64)*pos == xcdev->xcb->xpdev->ior_addr) {
		int rv;
		...
		while (0xffffffff == read_cash_user_reg(
			dev_hndl, CASH_SCRATCH_REG_CMD_STATUS_ADDR)) {
...
{
	struct qdma_cdev *xcdev = (struct qdma_cdev *)file->private_data;
	if (xcdev->qidx < CASH_VIDEO_QUEUE_COUNT && *pos == 0) {
		uint32_t req_reg_addr 
			= CASH_SCRATCH_REG_V_REQ_COUNT_ADDR + 4 * xcdev->qidx;
		if (write_cash_user_reg(xcdev->xcb->xpdev->dev_hndl
			, req_reg_addr
			, read_cash_user_reg(xcdev->xcb->xpdev->dev_hndl, req_reg_addr) + 1)) {
			return -EIO;
		}
	}
