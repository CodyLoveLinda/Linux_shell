...
static uint32_t const TEST_IO_Q_ID = 16;
static int cash_qdma_test_error = 0;

#undef QDMA_CASH_EXPECT
#define QDMA_CASH_EXPECT(x) if (!(x)) {\
	pr_err("QDMA_CASH_EXPECT %s is false @%s:%d\n", (#x), __FILE__, __LINE__); \
	cash_qdma_test_error |= -1;} else {\
	pr_debug("QDMA_CASH_CHECKED %s @%s:%d\n", (#x), __FILE__, __LINE__); \
    }

#undef QDMA_CASH_ASSERT
#define QDMA_CASH_ASSERT(x) if (!(x)) {\
    pr_err("QDMA_CASH_ASSERT %s is false @%s:%d\n", (#x), __FILE__, __LINE__); \
    cash_qdma_test_error |= -1; return;} else {\
	pr_debug("QDMA_CASH_CHECKED %s @%s:%d\n", (#x), __FILE__, __LINE__); \
    }

// #undef CASH_VIDEO_QUEUE_COUNT
// #define CASH_VIDEO_QUEUE_COUNT 2

// static
// int log_cash_fpga_events_func(void* dev) {
...
// 				case CASH_FPGA_EVENT_VIDEO_STATUS_CHANGED: {
// 					pr_info("CASH_FPGA_EVENT_VIDEO_STATUS_CHANGED\n");
// 				} break;
// 				case CASH_FPGA_EVENT_IMAGE_FLASHED: {
// 					pr_info("CASH_FPGA_EVENT_IMAGE_FLASHED\n");
// 				} break;
// 			}
// 		} else {
...
// 		schedule();
// 	}
// 	pr_info("CASH_FPGA_EVENT read stopping\n");
// 	do_exit(0);
// }
...
// static 
// struct task_struct *
// log_cash_fpga_events(unsigned long dev_hndl) {
// 	if (log_cash_fpga_events_thread == NULL) {
// 		char const our_thread[]="fpga_event_log";
// 		log_cash_fpga_events_thread 
// 			= kthread_create(log_cash_fpga_events_func, (void*)dev_hndl
// 			, our_thread);
// 		wake_up_process(log_cash_fpga_events_thread);
// 	}
// 	return log_cash_fpga_events_thread;
// }
...
/******************************************************************************/
/**
 * set_cash_checksum() -  setup checksum (XOR) on an array of memory chunks
 * @chunks: chunk array
...
static
uint32_t 
set_cash_checksum(void** chunks, size_t chunk_count, size_t chunk_size) {
	size_t i, j;
...
/******************************************************************************/
/**
 * verify_cash_checksum() -  verify checksum (XOR) on an array of memory chunks
...
static
uint32_t 
verify_cash_checksum(void** chunks, size_t chunk_count, size_t chunk_size) {
	size_t i, j;
	uint32_t checksum = 0, *addr = 0;
	for (i = 0; i < chunk_count; ++i) {
		addr = chunks[i];
	...
	QDMA_CASH_EXPECT(0 == checksum);
	if (checksum == 0) {
		QDMA_CASH_EXPECT(0 != *(addr - 1));
	}
	return *(addr - 1);
}
...
setup_tests(struct device *dev) {
	...
	struct cash_fpga_manifest_t* ver = (void*)&verInt;
	// uint32_t crc_buf[] = {0x1234, 0x5678, 0x9abc, 0xdef0, 0x1234, 0x5678, 0xbeef, 0xdead};

	pr_info("cash_fw_ver=%d file_fw_ver=%d, hw_ver=%d.%d"
		, ver->fw_ver
		, ver->backup_fw_ver
		, ver->hw_major_ver
		, ver->hw_minor_ver);

	// QDMA_CASH_EXPECT(NULL != log_cash_fpga_events(xpdev->dev_hndl));
	QDMA_CASH_EXPECT(0 == initialize_cash_video(xpdev->dev_hndl));
	for (i = 0; i < CASH_VIDEO_QUEUE_COUNT; ++i) {
		rv = clear_cash_video_status(xpdev->dev_hndl, i, 0);
		pr_info("video stream status %d = %d", i, rv);
		QDMA_CASH_ASSERT(0 == rv);
	}

	// pr_info("cash crc=%x", crc32(0x80000000 ^ 0xffffffff, (uint32_t*)crc_buf, 32) ^ 0xffffffff);
	// pr_info("cash crc=%x", crc32(0x80000000, (uint32_t*)crc_buf, 32));
	// {
	// 	//reset frame request size to be 6MB
	// 	//since most tests expect that
	// 	char mem[512] = {0};
	// 	struct cash_host_cmd_t *cmd = (void*)mem;
	// 	struct cash_host_cmd_set_video_params_t* set_params 
	// 		= (void*)cmd->payload;
	// 	uint32_t frs = 6 * 1024 * 1024;
	// 	cmd->head.cmd = CASH_HOST_CMD_VIDEO_SET_VIDEO_PARAMS;
	// 	cmd->head.cmd_count = CASH_VIDEO_QUEUE_COUNT;
	// 	for ( i = 0; i < CASH_VIDEO_QUEUE_COUNT; ++i) {
	// 		set_params->video_stream_id = i;
	// 		set_params++->frame_request_size = frs;
	// 	}

	// 	QDMA_CASH_EXPECT(0 < send_host_cmd(xpdev->dev_hndl
	// 		, cmd
	// 		, sizeof(mem)
	// 		, true)
	// 	);
	...
	// 	for (i = 0; i < CASH_VIDEO_QUEUE_COUNT; ++i) {
	// 		QDMA_CASH_ASSERT(0 == read_cash_user_reg(xpdev->dev_hndl
	// 			, CASH_SCRATCH_REG_CMD_STATUS_ADDR + 4 * i));
	// 	}
	// }
}
...
tear_down_tests(struct device *dev) {
	// struct xlnx_pci_dev * xpdev = dev_get_drvdata(dev);
	// if (log_cash_fpga_events_thread != NULL) {
	// 	QDMA_CASH_ASSERT(!kthread_stop(log_cash_fpga_events_thread));
	// 	log_cash_fpga_events_thread = NULL;
	// }
	// uninitialize_cash_video(xpdev->dev_hndl);
}
...
test_read_register(struct device *dev) {
	...
	uint32_t i;
	pr_debug("res = %x", res);
	QDMA_CASH_EXPECT(res == 0x1fD30000);

	res = read_cash_user_reg(xpdev->dev_hndl, 0);
	pr_debug("hw ver = 0x%x", res);

	for (i = 0; i < CASH_VIDEO_QUEUE_COUNT; ++i) {
		QDMA_CASH_EXPECT(0 
			== write_cash_user_reg(xpdev->dev_hndl
				, CASH_SCRATCH_REG_CMD_STATUS_ADDR + i * 4
				, 0xffffffff));
	}
	for (i = 0; i < CASH_VIDEO_QUEUE_COUNT; ++i) {
		QDMA_CASH_EXPECT(0xffffffff 
			== read_cash_user_reg(xpdev->dev_hndl
				, CASH_SCRATCH_REG_CMD_STATUS_ADDR + i * 4));
	}
}
...
static 
void 
test_cash_qdma_get_handles(struct device *dev) {
	struct xlnx_pci_dev * xpdev = dev_get_drvdata(dev);
	unsigned long dev_hndl, expected_qhndl, host_cmd_qhndl
		, fpga_event_qhndl
		, video_qhndls[CASH_VIDEO_QUEUE_COUNT] = {0};
	QDMA_CASH_EXPECT(0 == cash_qdma_get_handles(
		xpdev->idx, &dev_hndl, video_qhndls, 1, &host_cmd_qhndl
		, &fpga_event_qhndl));
	QDMA_CASH_EXPECT(0 != host_cmd_qhndl);
	QDMA_CASH_EXPECT(0 != fpga_event_qhndl);

	QDMA_CASH_EXPECT(xpdev->dev_hndl == dev_hndl);
	expected_qhndl = xpdev->qdata[xpdev->qmax].qhndl;
	QDMA_CASH_EXPECT(expected_qhndl == video_qhndls[0]);
}
...
test_prepare_unprepare_qdma(struct device *dev) {
	struct xlnx_pci_dev * xpdev = dev_get_drvdata(dev);
	int rv = 0;
	struct cash_qdma_request* qreq = kmalloc(sizeof(*qreq), GFP_KERNEL);
	...
	rv = cash_qdma_get_handles(xpdev->idx, &qreq->dev_hndl, NULL, 0
		, &qreq->qhndl, NULL);
	QDMA_CASH_ASSERT(rv == 0);
...
test_prepare_unprepare_video(struct device *dev) {
	struct xlnx_pci_dev * xpdev = dev_get_drvdata(dev);
	int rv = 0;
	struct cash_qdma_video_request* vreq = kmalloc(sizeof(*vreq), GFP_KERNEL);

	memset(vreq, 0, sizeof(*vreq));
	vreq->pdev = xpdev->pdev;
	vreq->buffer_chunk_count = 2;
	vreq->buffer_chunk_size = 3 * 1024 * 1024;

	rv = cash_qdma_get_handles(xpdev->idx, &vreq->dev_hndl, &vreq->qhndl, 1
		, NULL, NULL);
	QDMA_CASH_EXPECT(rv == 0);
	rv = prepare_cash_qdma_video_request(vreq);
	pr_debug("prepare_cash_qdma_video_request= %d", rv);
	QDMA_CASH_ASSERT(rv == 0);
...
test_io_area(struct device *dev) {
	...
	QDMA_CASH_EXPECT(ior_size > 100 * 1024 * 1024);
	QDMA_CASH_EXPECT(ior_addr);
	QDMA_CASH_EXPECT(0 == cash_qdma_get_handles(
		xpdev->idx, &dev_hndl, NULL, 0, &host_cmd_qhndl
		, NULL));
	QDMA_CASH_EXPECT(dev_hndl == xpdev->dev_hndl);
	pr_info("ior_addr=%llx", ior_addr);
	QDMA_CASH_ASSERT(0 != host_cmd_qhndl);
	...
	qreq.is_write = true;
	QDMA_CASH_EXPECT(0 == prepare_cash_qdma_request(&qreq));
	...
	checksum = set_cash_checksum(qreq.buffer_chunks
		, qreq.buffer_chunk_count, qreq.buffer_chunk_size);
	QDMA_CASH_EXPECT(qreq.buffer_chunk_count * qreq.buffer_chunk_size 
		== send_cash_qdma_request(&qreq));
	// dump_regs(dev_hndl);
	...
	qreq.is_write = false;
	qreq.qhndl = xpdev->qdata[TEST_IO_Q_ID  + xpdev->qmax].qhndl;
	QDMA_CASH_ASSERT(0 != qreq.qhndl);
	QDMA_CASH_EXPECT(0 == prepare_cash_qdma_request(&qreq));
	...
	for (i = 0; i < 100; ++i) {
		...
		QDMA_CASH_EXPECT(checksum == verify_cash_checksum(qreq.buffer_chunks
			, qreq.buffer_chunk_count, qreq.buffer_chunk_size));
	}
	unprepare_cash_qdma_request(&qreq);
}
...
test_sync_read_from_video_queue_zero(struct device *dev) {
	...
	struct cash_qdma_video_request vreq = {0};
	uint32_t const count = 6 * 1024 * 1024;
	struct timespec64 tv, tv2;
	int i;
	...
	rv = clear_cash_video_status(xpdev->dev_hndl, 0, 4);
	pr_info("video stream status %d = %d", 0, rv);

	rv = cash_qdma_get_handles(xpdev->idx, &dev_hndl, video_qhndls, 1
		, NULL, NULL);
	QDMA_CASH_EXPECT(rv == 0);
	...
	for (i = 0; i < 4; i++) {
		ktime_get_ts64(&tv);
		QDMA_CASH_EXPECT(count == send_cash_qdma_video_request(&vreq));
		ktime_get_ts64(&tv2);
	...
	QDMA_CASH_EXPECT( 0 == ((uint32_t*)(vreq.buffer_chunks[0]))[0]);
	unprepare_cash_qdma_video_request(&vreq);
}
...
aio_done_async_read_from_video_queue_zero(
	...
	struct cash_qdma_video_request** vreq_ptr = (void*)user_data;
	struct cash_qdma_video_request* vreq = *vreq_ptr;
	...
	QDMA_CASH_EXPECT(err == 0);
	QDMA_CASH_EXPECT(bytes_done == vreq->buffer_chunk_count *
...
test_async_read_from_video_queue_zero(struct device *dev) {
	struct cash_qdma_video_request* vreq = NULL;
	{
		...
		vreq->buffer_chunk_count = 3;
		vreq->buffer_chunk_size = 2 * 1024 * 1024;
		...
		vreq->dev_hndl = dev_hndl;
		vreq->qhndl = video_qhndls[0];
		QDMA_CASH_EXPECT(0 == prepare_cash_qdma_video_request(vreq));

		pr_debug("==ready to submit===\n");
		QDMA_CASH_EXPECT(0 == send_cash_qdma_video_request(vreq));

		pr_debug("==done submit===\n");
	}
	msleep_interruptible(1000);
	QDMA_CASH_EXPECT(vreq == NULL);
	pr_debug("vreq=%p\n", vreq);
}
...
aio_done_async_read_multi_from_video_queue_zero(
	...
	pr_debug("%s: bytes_done=%d, err=%d, buffer_chunk_count=%ld, buffer_chunk_size=%ld"
		, __FUNCTION__, bytes_done, err, vreq->buffer_chunk_count, vreq->buffer_chunk_size);
	QDMA_CASH_EXPECT(err == 0);
	QDMA_CASH_EXPECT(bytes_done == vreq->buffer_chunk_count * vreq->buffer_chunk_size);
	QDMA_CASH_EXPECT( 0 == ((uint32_t*)(vreq->buffer_chunks[0]))[0]);
	unprepare_cash_qdma_video_request(vreq);
	kfree(vreq);
	*vreq_ptr = NULL;
...
test_async_multiple_read_from_video_queue_zero(struct device *dev) {
	...
	while (test_frame_count--) {
		...
		vreq->dev_hndl = dev_hndl;
		vreq->qhndl = video_qhndls[0];
		QDMA_CASH_ASSERT(0 == prepare_cash_qdma_video_request(vreq));

		pr_debug("==ready to submit===\n");
		QDMA_CASH_ASSERT(0 == send_cash_qdma_video_request(vreq));

		pr_debug("==done submit===\n");

		msleep_interruptible(200);
		QDMA_CASH_ASSERT(vreq == NULL);
	}
}
...
test_async_multiple_read_from_video_queue_zero_reuse_req(struct device *dev) {
	int test_frame_count = 29;
	{
		struct cash_qdma_video_request * volatile vreq = 0, *vreq_keep;
		struct xlnx_pci_dev *xpdev = dev_get_drvdata(dev);
		int rv = 0;
		unsigned long dev_hndl, qhndl, video_qhndls[1] = {0};
		vreq = kzalloc(sizeof(*vreq), GFP_KERNEL);
		...
		unprepare_cash_qdma_video_request(vreq_keep);
		kfree(vreq);
	}
}

struct my_video_request {
	struct cash_qdma_video_request vreq;
};
...
aio_done_async_multiple_read_from_all_video_queues_reuse_req(
	...
	struct cash_qdma_video_request* vreq = &((*my_vreq_ptr)->vreq);
...
async_multiple_read_from_all_video_queues_reuse_req(struct device *dev
	...
	struct xlnx_pci_dev *xpdev = dev_get_drvdata(dev);
	uint32_t qid;
	unsigned long dev_hndl, video_qhndls[CASH_VIDEO_QUEUE_COUNT] = {0};
	struct my_video_request * volatile my_vreqs[CASH_VIDEO_QUEUE_COUNT] = {0}
		, *my_vreqs_keep[CASH_VIDEO_QUEUE_COUNT] = {0};
	QDMA_CASH_ASSERT(0 == cash_qdma_get_handles(
		xpdev->idx, &dev_hndl, video_qhndls, CASH_VIDEO_QUEUE_COUNT
		, NULL, NULL));
	for (qid = 0; qid < CASH_VIDEO_QUEUE_COUNT; ++qid) {
		struct cash_qdma_video_request *vreq = 0;
		my_vreqs[qid] = kzalloc(sizeof(*my_vreqs[qid]), GFP_KERNEL);
		QDMA_CASH_ASSERT(0 != my_vreqs[qid]);
	...
	for (i = 0; i < test_frame_count; ++i) {
		for (qid = 0; qid != CASH_VIDEO_QUEUE_COUNT; ++qid) {
			struct cash_qdma_video_request *vreq = &my_vreqs_keep[qid]->vreq;
		...
		for (qid = 0; qid < CASH_VIDEO_QUEUE_COUNT; ++qid) {
			...
			QDMA_CASH_EXPECT(my_vreqs[qid] == NULL);
		}

		if ((i + 1) % 100 == 0) {
			pr_info("%d streams, done_frame_count_per_stream = %d", CASH_VIDEO_QUEUE_COUNT, (i + 1));
		}
	}

	for (qid = 0; qid < CASH_VIDEO_QUEUE_COUNT; ++qid) {
		unprepare_cash_qdma_video_request(&my_vreqs_keep[qid]->vreq);
...
test_async_multiple_read_from_all_video_queues_reuse_req_rate_check(struct device *dev) {
	...
	async_multiple_read_from_all_video_queues_reuse_req(dev
		, 60 * 10);
	ktime_get_ts64(&end);
	QDMA_CASH_EXPECT(end.tv_sec - start.tv_sec < 62);
}
...
test_async_multiple_display_all_video_queues_reuse_req(struct device *dev) {
	...
	for (i = 0; i < test_frame_count; ++i) {
		...
		if ((i + 1) % 100 == 0) {
			pr_info("done_frame_count = %d", (i + 1) * CASH_VIDEO_QUEUE_COUNT);
		}
	}
...
test_cmd_event_cycle(struct device *dev) {
	...
	char mem[512] = {0};
	struct cash_host_cmd_t *cmd = (void*)mem;
	struct cash_host_cmd_trigger_dummy_event_t* trigger 
		= (void*)cmd->payload;
	...
	rv = cash_qdma_get_handles(xpdev->idx, &dev_hndl, NULL, 0
		, &cmd_qhndl, &event_qhndl);
	QDMA_CASH_ASSERT(rv == 0);
	QDMA_CASH_ASSERT(event_qhndl != 0);
	QDMA_CASH_ASSERT(cmd_qhndl != 0);
	
	cmd->head.cmd = CASH_HOST_CMD_TRIGGER_DUMMY_EVENT;
	...
	for (; i < 100; ++i) {
		...
		QDMA_CASH_EXPECT(0 < rv);
		rv = send_cash_qdma_request(&qreq);
		pr_info("send_cash_qdma_request rv=%d", rv);
		QDMA_CASH_EXPECT(qreq.buffer_chunk_size == rv);
		ktime_get_ts64(&tv2);

		display_time("cash_host_cmd_trigger_dummy_event_t sent@", &tv);
		display_time("cash_fpga_event_dummy_t             recv@", &tv2);
		QDMA_CASH_EXPECT(CASH_FPGA_EVENT_DUMMY == event->head.event);
		pr_debug("event->head.event=%d", event->head.event);
		{
			struct cash_fpga_event_dummy_t* dummy = (void*)&event->payload;
			QDMA_CASH_EXPECT(dummy->backReference == trigger->backReference);
...
test_cmd_event_cycle_timing(struct device *dev) {
	...
	for (; i < 100; ++i) {
		rv = send_cash_qdma_request(&qreq);
		pr_info("send_cash_qdma_request rv=%d", rv);
		QDMA_CASH_EXPECT(rv == 0); //async
		// msleep_interruptible(100);

		QDMA_CASH_EXPECT(0 
			== write_cash_user_reg(xpdev->dev_hndl
				, CASH_SCRATCH_REG_START_ADDR + 62 * 4
				, 0x0));
		...
		while(0 == read_cash_user_reg(xpdev->dev_hndl, CASH_SCRATCH_REG_START_ADDR + 62 * 4)) {
		...
		display_time("cash_host_cmd_trigger_dummy_event_t   pre-sent@", &tv);
		display_time("cash_host_cmd_trigger_dummy_event_t  post-sent@", &tv_sent);
...
test_qdma_timing(struct device *dev) {
	...
	for (; i < 100; ++i) {
	    ...
	    // QDMA_CASH_EXPECT(CASH_FPGA_EVENT_DUMMY == event->hea
	}
	unprepare_cash_qdma_request(&qreq);
}
...
// 	struct xlnx_pci_dev * xpdev = dev_get_drvdata(dev);
// 	int rv = 0;
// 	unsigned long dev_hndl, qhndl, video_qhndls[CASH_VIDEO_QUEUE_COUNT] = {0};
// 	struct cash_qdma_video_request vreq = {0};
// 	vreq.pdev = xpdev->pdev;
// 	vreq.buffer_chunk_count = 2;
// 	vreq.buffer_chunk_size = frame_request_size / 2;

// 	rv = cash_qdma_get_handles(xpdev->idx, &dev_hndl
// 		, video_qhndls, CASH_VIDEO_QUEUE_COUNT, NULL, NULL);
// 	QDMA_CASH_EXPECT(rv == 0);
...
// 	// qdma_sw_sg_dump(vreq.reserved->sgl);
// 	while (count--) {
// 		QDMA_CASH_EXPECT(frame_request_size 
// 			== send_cash_qdma_video_request(&vreq));
// 	} 
// 	QDMA_CASH_EXPECT(video_id == ((uint32_t*)(vreq.buffer_chunks[0]))[0]);
// 	unprepare_cash_qdma_video_request(&vreq);
// }
...
// 	QDMA_CASH_ASSERT(0 < send_host_cmd(xpdev->dev_hndl
// 		, cmd
// 		, sizeof(mem)
// 		, true)
// 	);
...
// 	QDMA_CASH_ASSERT(0 == read_cash_user_reg(xpdev->dev_hndl
// 		, CASH_SCRATCH_REG_CMD_STATUS_ADDR + 4));
...
int sync_video_stream_func(void* param_in) {
	...
	QDMA_CASH_EXPECT(dest_mem != NULL);
	...
	rv = cash_qdma_get_handles(xpdev->idx, &dev_hndl, video_qhndls, CASH_VIDEO_QUEUE_COUNT
		, NULL, NULL);
	QDMA_CASH_EXPECT(rv == 0);
	...
	vreq.qhndl = qhndl;
	rv = prepare_cash_qdma_video_request(&vreq);
	pr_debug("v_id=%d, prepare_cash_qdma_video_request= %d", v_id, rv);
	QDMA_CASH_EXPECT(rv == 0);

	ktime_get_ts64(&tv);
	for (i = 0; i < count; i++) {
		struct cash_video_meta_data_t* meta = vreq.buffer_chunks[0];
		uint32_t ts_gap_us = (meta->curr_ts_sec - meta->prev_ts_sec) * 1000000 
			+ meta->curr_ts_us - meta->prev_ts_us;
		ssize_t send_count = send_cash_qdma_video_request(&vreq);
		QDMA_CASH_EXPECT(6 * 1024 * 1024 == send_count);
		if (6 * 1024 * 1024 != send_count) {
			pr_err("v_id %d exiting abnormally", v_id);
			return -1;
		}
		QDMA_CASH_EXPECT(v_id == meta->mipi_id);
		QDMA_CASH_EXPECT(ts_gap_us < 120000);
		if  (ts_gap_us >= 120000) {
...
test_sync_threads(struct device *dev) {
	struct sync_video_stream_t params[CASH_VIDEO_QUEUE_COUNT];
	uint16_t v_id = 0;
	uint32_t count = 10 * 60 * 2;

	for (;v_id < CASH_VIDEO_QUEUE_COUNT; ++v_id) {
		char const our_thread[]="fpga_event_log";
...
/*3*/	ADD_TEST(test_cash_qdma_get_handles),
...
ssize_t show_libqdma_cash_test_cases(struct device *dev,
...
ssize_t run_libqdma_cash_test_case(struct device *dev,
	...
	cash_qdma_test_error = 0;
	tests[test_case_id].test_case(dev);
	pr_info("test %d) ==> %d  %s\n", test_case_id, cash_qdma_test_error, tests[test_case_id].name);
	return count;
}
