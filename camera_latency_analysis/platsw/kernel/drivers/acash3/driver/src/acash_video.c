...
#define	VLOG(f, v, fmt, ...)	f("q%u video%d %s: " fmt, \
	v->qidx, (signed short)v->vdev.num, v->caps.name, ## __VA_ARGS__)
...
static int autox_video_packet_request(autox_video_t *avideo)
{
	...
	rv = cash3_qdma_get_handles(xpdev->idx, &dev_hndl,
				    video_qhndls, queue_spec->video.count,
				    NULL, NULL);
	...
	if (prepare_cash3_qdma_video_request(vreq) != 0)
	{
		VLOG_ERR(avideo, "video qdma request failed \n");
		return -1;
	}
	VLOG_DEBUG(avideo,
		"==sizeimage (%u), \
...
static int autox_video_enable(autox_video_t *avideo)
{
	struct xlnx_pci_dev *xpdev;
	int rv = 0;
	...
	rv = clear_cash3_video_status(xpdev->dev_hndl, avideo->qidx, 0);
	if (rv != 0) {
		VLOG_ERR(avideo, "clear_cash3_video_status ERR (%d)\n", rv);
	}
	...
	if (rv != 0) {
		VLOG_ERR(avideo, "autox_video_packet_request ERR (%d)\n", rv);
	}
	return rv;
}

struct cash3_video_meta_data_t g_meta_data[CASH3_VIDEO_QUEUE_COUNT_MAX] = {0};

int autox_video_check_meta_data(autox_video_t *avideo,
				autox_video_buffer_t *buf,
				struct cash3_video_meta_data_t *metadata)
{
	...
	unsigned int cur_a53_idx = 0;
	unsigned int a53_idx = 0;
	unsigned int epochtime_sec = 0;
	unsigned int epochtime_us = 0;
#ifdef EPOCH_PPS_LOCK
	int sec_diff = 0;
#endif	
	...
	cur_a53_idx = metadata->mipi_frame_idx & 0xFF;
	a53_idx = (g_meta_data[avideo->qidx].mipi_frame_idx) & 0xFF;

	epochtime_sec = metadata->epochtime_sec;
	epochtime_us = metadata->epochtime_us;
...
/*
   4
   Copy bit of bit 32
   Always equal bit 32, which is the last bit of stardard epochTime. It could
   indicate a rollover condition if two separate 32-bit reading are performed.
   3
   TIME_DIFF_FLAG
   2
   PPS_LOCK
   ...
   MSG_RCVD
*/
#ifdef EPOCH_COPY_BIT	
	if ((epochtime_sec & 0x1) != (epochtime_us >> 4 & 0x1)) {
		epochtime_sec += 1;
		printk(KERN_ERR "epochtime_sec rollover== (%d) (%u)\n",
		       avideo->qidx, epochtime_sec);
	}
#endif	

	tv.tv_sec = epochtime_sec;
	//tv.tv_usec = (epochtime_us >> 15 ) * 10;
    tv.tv_nsec = (epochtime_us >> 15 ) * 10 * NSEC_PER_USEC;


#ifdef EPOCH_PPS_LOCK
	/* No PPS Lock */
	if (!(epochtime_us & 0x4)) {
		/* If host time seconds and epoch time seconds are different
		 * more than 2 seconds then use host time
		 */
		sec_diff = avideo->tv_end.tv_sec = epochtime_sec;
		if ((sec_diff > 2) || (sec_diff < -2)) {
			tv = avideo->tv_end;
		}
	}
#endif
...
/*
	...
	buf->vbuf.vb2_buf.timestamp = timeval_to_ns(&tv);
#else
	buf->vbuf.timestamp = tv;
#endif
*/
	...
	{
		VLOG_DEBUG(avideo,
			"A53 Frame Drop== (%d) mipi_frame_idx(%x) \
			cur_a53_idx(%x), a53_idx(%x), req(%x) resp(%x)\n", \
			avideo->qidx, \
			g_meta_data[avideo->qidx].mipi_frame_idx, \
			cur_a53_idx, a53_idx, \
			metadata->req, metadata->resp);
	}

	g_meta_data[avideo->qidx].mipi_frame_idx = cur_a53_idx;
	g_meta_data[avideo->qidx].resp = metadata->resp;
	g_meta_data[avideo->qidx].req = metadata->req;
...
static int autox_video_buffer_prepare(struct vb2_buffer *vb)
{
	...
	VLOG_DEBUG(avideo, "vb 0x%p, index %u avideo->fmt_idx %d  SIZE %d  size %d \n", 
		vb, vb->index, avideo->fmt_idx, bufsz, size);
		...
		struct v4l2_fmtdesc *fmtdesc)
{
	...
	VLOG_DEBUG(avideo, "VIDIOC_ENUM_FMT: file 0x%p, index %u\n",
			file, fmtdesc->index);
	...
	VLOG_INFO(avideo, "pixelformat %c%c%c%c\n",
		...
		struct v4l2_format *fmt)
{
	...
	VLOG_DEBUG(avideo, "pixelformat %c%c%c%c, width %u, height %u\n",
		...
		struct v4l2_frmsizeenum *fsize)
{
	...
	VLOG_DEBUG(avideo, "width %u, height %u\n",
...
static void autox_video_format_init(autox_video_t *avideo)
{
	...
	autox_scratch_reg_addr_spec_t *scratch_reg = &((autox_spec_t *)avideo->xpdev->spec)->scratch_reg;
	uint32_t video_state_shift = 4 * (avideo->qidx % 8);
	...
	video_state_addr = scratch_reg->video_state.addr + (avideo->qidx / 8) * 4;
	video_state = (read_cash3_user_reg(avideo->xpdev->dev_hndl, video_state_addr) >> video_state_shift) & 0xf;
	VLOG_INFO(avideo, "video_state_addr 0x%X    video_state %x ", video_state_addr, video_state);

	switch(video_state) {
		case CASH3_VIDEO_STATE_READY_RGB_2M:
			avideo->fmt_idx = 3;
			break;
		case CASH3_VIDEO_STATE_READY_RGB_8M:
			avideo->fmt_idx = 2;
			break;
		case CASH3_VIDEO_STATE_READY_UYVY_8M:
			avideo->fmt_idx = 1;
			break;
		case CASH3_VIDEO_STATE_READY_YUYV_8M:
			avideo->fmt_idx = 0;
			break;
		default:
			avideo->fmt_idx = 2;
			VLOG_ERR(avideo," autox_video_format_init unsupported video state %d,  please check !!! \n", video_state);
			break;
	}
...
static int autox_video_check_link(autox_video_t *avideo)
{
	...
	unsigned char prev;
	int changed;
	autox_spec_t *aspec = (autox_spec_t *)avideo->xpdev->spec;
	...
	if (video_state == CASH3_VIDEO_STATE_NOT_READY)
		avideo->link_up = 0;
	else
		avideo->link_up = video_state;

	changed = (avideo->link_up != prev);
	if (changed) {
		VLOG_INFO(avideo, "camera link ~~~~~ %d %s\n", avideo->link_up,
...
static void autox_video_link_change(autox_video_t *avideo)
{
	...
	if (avideo->state & VIDEO_STATE_STREAMING) {
		rv = send_cash3_qdma_video_request(avideo->vreq);
		autox_video_packet_copy((unsigned long)avideo, rv, 0);
	}
	else {
		/* No need this logic with DMA setup with video node register
		if (avideo->vreq) {
			unprepare_cash3_qdma_video_request(avideo->vreq);
...
int autox_video_register_device(autox_video_t *avideo)
{
	...
	//// qdma hardware video channel setup ///////////////////////////////
	avideo->buf_total = VIDEO_BUF_MIN_NUM;
	autox_video_init_params(avideo);
	ret = autox_video_enable(avideo);
	//////////////////////////////////////////////////////////////////////
	if (ret!= 0)
	{
		VLOG_ERR(avideo, " autox_video_register_device failed due to qDMA setup error \n");
		return -1;
	}
...
/*
 * There are 8 or 16 cameras at Max based on FPGA FW 
 * There are up to 8 or 16 V4L2 device video nodes
*/
int autox_video_alloc(struct xlnx_pci_dev *xpdev)
{
	...
	pr_info("autox_video_alloc   xpdev->video_cnt %d \n", xpdev->video_cnt);
		...
		unsigned int bytes_done, int err)
{
	...
	struct cash3_qdma_video_request *vreq = avideo->vreq;
	unsigned int sizeimage =
		autox_video_formats[avideo->fmt_idx].pix.sizeimage;
	...
	if (autox_video_check_meta_data(avideo, buf,
			(struct cash3_video_meta_data_t *)meta_data)) {
