...
#ifdef AUTOX
static ssize_t show_host_cmd(struct device *dev,
	struct device_attribute *attr, char *buf) 
{
	char* buf_old = buf;
	buf += sprintf(buf, "a write to this device trigger "
		"the FPGA to process the cmd stored in the IO area\n");
	return buf - buf_old;
}
	...
	struct device_attribute *attr, const char *buf, size_t count) 
{
	...
	if (write_cash_user_reg(xpdev->dev_hndl, CASH_FPGA_IRQ_STATUS_ADDR
	...
	struct device_attribute *attr, char *buf) 
{
	...
	video_state = 
		(read_cash_user_reg(xpdev->dev_hndl, CASH_SCRATCH_REG_VIDEO_STATE_ADDR));
	...
	struct device_attribute *attr, char *buf) 
{
	...
	fw_ver = 
		(read_cash_user_reg(xpdev->dev_hndl, CASH_SCRATCH_REG_FPGA_VER_ADDR)) & 0x3FF;
...
static DEVICE_ATTR(test_case, S_IWUSR | S_IRUGO,
			show_libqdma_cash_test_cases, run_libqdma_cash_test_case);		
...
#define TEST
	...
	initialize_cash_video(xpdev->dev_hndl);
