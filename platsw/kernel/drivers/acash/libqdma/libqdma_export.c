...
void qdma_sw_sg_dump(struct qdma_sw_sg* sgl) {
	while (sgl) {
		pr_info("qdma_sw_sg=%p, next=%p, page=%p, offset=%d, len=%d, dma_addr=%lld\n"
			, sgl, sgl->next, sgl->pg, sgl->offset, sgl->len, sgl->dma_addr);
		sgl = sgl->next;
	}
}
...
void qdma_dump_dev_conf(struct qdma_dev_conf* conf) {
	pr_info("qdma_dev_conf pdev = %p\n",     conf->pdev);
	pr_info("qsets_max          = %d\n",     conf->qsets_max);
	pr_info("rsvd2              = %d\n",     conf->rsvd2);
	pr_info("zerolen_dma        = %d\n",     conf->zerolen_dma );
	pr_info("master_pf          = %d\n",     conf->master_pf   );
	pr_info("rsvd1              = %d\n",     conf->rsvd1   );
	pr_info("vf_max             = %d\n",     conf->vf_max  );
	pr_info("intr_rngsz         = %d\n",     conf->intr_rngsz  );
	pr_info("msix_qvec_max      = %d\n",     conf->msix_qvec_max);
	pr_info("uld                = %lu\n",    conf->uld );
	pr_info("qdma_drv_mode      = %d\n",     conf->qdma_drv_mode);
	pr_info("name               = %s\n",     conf->name);

#ifdef RTL2_FEATURE_CONFIGURABLE_BAR
	pr_info("bar_num_config     = %d\n",     conf->bar_num_config);
	pr_info("bar_num_user       = %d\n",     conf->bar_num_user);
	pr_info("bar_num_bypass     = %d\n",     conf->bar_num_bypass);
#endif
	pr_info("bar_num_config     = %d\n",     conf->bar_num_config);
	pr_info("bar_num_user       = %d\n",     conf->bar_num_user);
	pr_info("bar_num_bypass     = %d\n",     conf->bar_num_bypass);
	pr_info("qsets_base         = %d\n",     conf->qsets_base);
	pr_info("bdf                = %d\n",     conf->bdf);
	pr_info("idx                = %d\n",     conf->idx);
	pr_info("fp_user_isr_handler= %p\n",     conf->fp_user_isr_handler);
	pr_info("fp_q_isr_top_dev   = %p\n",     conf->fp_q_isr_top_dev);
	pr_info("debugfs_dev_root   = %p\n",     conf->debugfs_dev_root);
}

void qdma_dump_queue_conf(struct qdma_queue_conf* qconf) {
	pr_info("qdma_queue_conf of qidx=%d\n", qconf->qidx);
	pr_info("st                     =%d\n", qconf->st);
	pr_info("c2h                    =%d\n", qconf->c2h);
	pr_info("pipe                   =%d\n", qconf->pipe);
	pr_info("irq_en                 =%d\n", qconf->irq_en);
	pr_info("desc_rng_sz_idx        =%d\n", qconf->desc_rng_sz_idx);
	pr_info("wb_status_en           =%d\n", qconf->wb_status_en);
	pr_info("cmpl_status_acc_en     =%d\n", qconf->cmpl_status_acc_en);
	pr_info("cmpl_status_pend_chk   =%d\n", qconf->cmpl_status_pend_chk);
	pr_info("desc_bypass            =%d\n", qconf->desc_bypass);
	pr_info("pfetch_en              =%d\n", qconf->pfetch_en);
	pr_info("fetch_credit           =%d\n", qconf->fetch_credit);
	pr_info("st_pkt_mode            =%d\n", qconf->st_pkt_mode);
	pr_info("c2h_buf_sz_idx         =%d\n", qconf->c2h_buf_sz_idx);
	pr_info("cmpl_rng_sz_idx        =%d\n", qconf->cmpl_rng_sz_idx);
	pr_info("cmpl_desc_sz           =%d\n", qconf->cmpl_desc_sz);
	pr_info("cmpl_stat_en           =%d\n", qconf->cmpl_stat_en);
	pr_info("cmpl_udd_en            =%d\n", qconf->cmpl_udd_en);
	pr_info("cmpl_timer_idx         =%d\n", qconf->cmpl_timer_idx);
	pr_info("cmpl_cnt_th_idx        =%d\n", qconf->cmpl_cnt_th_idx);
	pr_info("cmpl_trig_mode         =%d\n", qconf->cmpl_trig_mode);
	pr_info("cmpl_en_intr           =%d\n", qconf->cmpl_en_intr);
	pr_info("sw_desc_sz             =%d\n", qconf->sw_desc_sz);
	pr_info("pfetch_bypass          =%d\n", qconf->pfetch_bypass);
	pr_info("cmpl_ovf_chk_dis       =%d\n", qconf->cmpl_ovf_chk_dis);
	pr_info("port_id                =%d\n", qconf->port_id);
	pr_info("at                     =%d\n", qconf->at);
	pr_info("init_pidx_dis          =%d\n", qconf->init_pidx_dis);
	pr_info("en_mm_cmpt             =%d\n", qconf->en_mm_cmpt);
	pr_info("cdh_max                =%d\n", qconf->cdh_max);
	pr_info("pipe_gl_max            =%d\n", qconf->pipe_gl_max);
	pr_info("pipe_flow_id           =%d\n", qconf->pipe_flow_id);
	pr_info("pipe_slr_id            =%d\n", qconf->pipe_slr_id);
	pr_info("pipe_tdest             =%d\n", qconf->pipe_tdest);
	pr_info("quld                   =%lu\n",qconf-> quld);
	pr_info("fp_descq_isr_top       =%p\n", qconf->fp_descq_isr_top);
	pr_info("fp_descq_c2h_packet    =%p\n", qconf->fp_descq_c2h_packet);
	pr_info("fp_bypass_desc_fill    =%p\n", qconf->fp_bypass_desc_fill);
	pr_info("fp_proc_ul_cmpt_entry  =%p\n", qconf->fp_proc_ul_cmpt_entry);
	pr_info("name                   =%s\n", qconf->name);
	pr_info("rngsz                  =%d\n", qconf->rngsz);
	pr_info("rngsz_cmpt             =%d\n", qconf->rngsz_cmpt);
	pr_info("c2h_bufsz              =%d\n", qconf->c2h_bufsz);
}
			...
			struct qdma_request *req)
{
	...
	pr_debug("%s: cb 0x%p %u,%u submitted.\n", descq->conf.name, cb, cb->offset, cb->left);
