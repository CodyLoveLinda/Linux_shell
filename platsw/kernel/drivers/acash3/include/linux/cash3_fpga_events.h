/*
 * This file captures the CASH3 FPGA events reported to hosts
 *
 ...
 */
#ifndef __CASH3_FPGA_EVENTS_H__
#define __CASH3_FPGA_EVENTS_H__
...
#define CASH3_FPGA_EVENT_QDMA_QUEUE_ID_C8	8
#define CASH3_FPGA_EVENT_QDMA_QUEUE_ID_C16	16
#define CASH3_FPGA_EVENT_SIZE			4096

/**
 * @enum - cash3_fpga_event_en
 * @brief	event tags - do not reused the released tag numbers - keep adding
 * at the end instead
 */
enum cash3_fpga_event_en {
	CASH3_FPGA_EVENT_VIDEO_STATUS_CHANGED 		= 1,
	CASH3_FPGA_EVENT_IMAGE_FLASHED				= 2,
	CASH3_FPGA_EVENT_HEALTH_REPORT				= 3,
	CASH3_FPGA_EVENT_ERROR_REPORT				= 4,
	CASH3_FPGA_EVENT_CAN_TRAFFIC					= 5,
	CASH3_FPGA_EVENT_DUMMY						= 0xffff, //no more events
};

/**
 * @struct - cash3_fpga_event_head_t
 * @brief	every event start with this header
 */
struct cash3_fpga_event_head_t {
	enum cash3_fpga_event_en event : 16;	//what event
...
struct cash3_fpga_event_t {
	struct cash3_fpga_event_head_t head;
	uint8_t payload[CASH3_FPGA_EVENT_SIZE - sizeof(struct cash3_fpga_event_head_t)];
} __attribute__((packed));

/**
 * @struct - cash3_fpga_event_image_flashed_t
...
/**
 * @enum - cash3_video_status_event_en
 * @brief	used to show the new status of a single video queue (stream)
 * FPGA writes it and host reads it
 */
enum cash3_video_status_event_en {
	CASH3_VIDEO_STATUS_EVENT_NOT_READY = 0,
	CASH3_VIDEO_STATUS_EVENT_READY,
};

/**
 * @struct - cash3_fpga_event_video_status_changed_t
 * @brief	a video stream status has changed
 */
struct cash3_fpga_event_video_status_changed_t {
	uint8_t video_stream_id;
	uint8_t reserved;
	enum cash3_video_status_event_en change : 16;
 ...
 * @brief
 */
struct cash3_fpga_event_fd_link_status_t {
	uint8_t cam_id;
...
struct cash3_mipi_intc_status_t {
	uint32_t isr;
	uint32_t ipr;
	uint32_t ier;
	uint32_t mer;
};
struct cash3_mipi_top_status_t {
	uint32_t fpgaVersion;
	uint32_t status;
};

struct cash3_mipi_frmwr_status_t {
	uint32_t control;
	uint32_t iis;
};

struct cash3_mipi_csc_status_t {
	uint32_t control;
	uint32_t iis;
};

struct cash3_mipi_crs_status_t {
	uint32_t control;
	uint32_t iis;
};
struct cash3_mipi_tpg_status_t {
	uint32_t control;
	uint32_t iis;
};

struct cash3_mipi_csi_status_t {
	uint32_t ccr;
	uint32_t pcr;
	uint32_t csr;
	uint32_t isr;
 ...
 * @brief
 */
struct cash3_fpga_event_mipi_status_t {
	uint32_t mipi_id:8;
	uint32_t state:8;				//ACTIVE, IDLE, DEFECTED
	uint32_t reserved:16;
	struct cash3_mipi_log_t log;
	struct cash3_mipi_intc_status_t intc;
	struct cash3_mipi_top_status_t top;
	struct cash3_mipi_frmwr_status_t frmwr;
	struct cash3_mipi_crs_status_t crs;
	struct cash3_mipi_csc_status_t csc;
	struct cash3_mipi_tpg_status_t tpg;
	struct cash3_mipi_csi_status_t csi;
} __attribute__((packed));
/**
 * @struct -
 * @brief
 */
struct cash3_fpga_event_fpga_status_t {
	uint64_t video_state;
} __attribute__((packed));
/**
 * @struct -
 * @brief
 */
struct cash3_fpga_event_misc_status_t {
	uint32_t core_temperature;
	uint32_t reserved;
} __attribute__((packed));

struct cash3_fpga_event_mipi_monitor_status_t {
    uint16_t idx;
    uint16_t state;
    uint16_t idleCount;     //number of monitor timer occurences while frameIdx not changed
    uint16_t maxIdle;
    uint32_t dummyFrames;   //Total number of dummy frames generated
    uint32_t recoveryTimes; //MIPI recpvery time
} __attribute__((packed));
 ...
 * @brief
 */
struct cash3_fpga_event_health_status_t {
	uint64_t ts_sec;
	uint64_t ts_us;
	struct cash3_fpga_event_misc_status_t misc_status;
	struct cash3_fpga_event_fpga_status_t fpga_status;
	struct cash3_fpga_event_fd_link_status_t fd_link_status[CASH3_VIDEO_QUEUE_COUNT_MAX];
	struct cash3_fpga_event_mipi_monitor_status_t mipi_monitor[CASH3_VIDEO_QUEUE_COUNT_MAX];
	struct cash3_fpga_event_mipi_status_t mipi_status[CASH3_VIDEO_QUEUE_COUNT_MAX];
} __attribute__((packed));
 ...
 * @brief
 */
struct cash3_fpga_event_dummy_t {
	uint64_t backReference;		//see cash3_host_cmd_trigger_dummy_event_t
...
// /**
//  * @struct - cash3_fpga_event_cmd_failed_t
