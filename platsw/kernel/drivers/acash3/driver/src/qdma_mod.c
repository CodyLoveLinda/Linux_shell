...
#include "linux/cash3_basic_registers.h"

#include "nl.h"
#include "libqdma/xdev.h"
#include "libqdma_cash3.h"
#include "libqdma_cash3_tests.h"
...
static unsigned int qdma_test = 0;
module_param(qdma_test, uint, 0644);
MODULE_PARM_DESC(qdma_test, "option to enable qdma test cases");
...
#ifdef AUTOX
static
ssize_t show_host_cmd(struct device *dev,
       struct device_attribute *attr, char *buf) {
       char* buf_old = buf;
       buf += sprintf(buf, "a write to this device trigger the FPGA to process the cmd stored in the IO area\n");
       return buf - buf_old;
}

static ssize_t trigger_host_cmd(struct device *dev,
       struct device_attribute *attr, const char *buf, size_t count) {
       struct xlnx_pci_dev *xpdev;
       xpdev = (struct xlnx_pci_dev *)dev_get_drvdata(dev);
       if (!xpdev) {
               pr_err("invalid xpdev - AUTOX qdma internal error");
               return count;
       }
       if (write_cash3_user_reg(xpdev->dev_hndl, CASH3_FPGA_IRQ_STATUS_ADDR
               , CASH3_HOST_CMD_PENDING)) {
               pr_err("invalid xpdev - AUTOX pci internal error");
       }
       return count;
}

static ssize_t show_video_sts(struct device *dev,
	struct device_attribute *attr, char *buf) 
{
	...
	len = sprintf(buf, "0x");
	for (i = scratch_reg->video_state.count - 1; i >= 0; i--) {
		video_state = read_cash3_user_reg(xpdev->dev_hndl, scratch_reg->video_state.addr + i * 4);
		len += sprintf(buf +len, "%08x", video_state);
	}
	...
	struct device_attribute *attr, char *buf) 
{
	...
	scratch_reg = &((autox_spec_t *)xpdev->spec)->scratch_reg;
	fw_ver = (read_cash3_user_reg(xpdev->dev_hndl, 
		scratch_reg->fpga_ver.addr)) & 0x3FF;
	len = sprintf(buf, "FW Ver: %d\n", fw_ver);
	return len;
}

static ssize_t show_hw_status(struct device *dev, 
	struct device_attribute *attr, char *buf)
{
	struct xlnx_pci_dev *xpdev;
	uint32_t reg_val0;
	...
	reg_val0 = read_cash3_user_reg(xpdev->dev_hndl, CASH3_SPARE_STATUS_0_REG);
	reg_val1 = read_cash3_user_reg(xpdev->dev_hndl, CASH3_SPARE_STATUS_0_REG + 4);
	len += sprintf(buf + len, "spare status reg0: 0x%x\n", reg_val0);
	len += sprintf(buf + len, "spare status reg1: 0x%x\n\n", reg_val1);
	len += sprintf(buf + len, "Message Received: %u\n", (reg_val0 & 0x1));
	len += sprintf(buf + len, "Valid Or Warning Byte: %u\n", (reg_val0 >> 1) & 0x1);
	len += sprintf(buf + len, "Pps Lock: %u\n", (reg_val0 >> 2) & 0x1);
	len += sprintf(buf + len, "Time Diff Flag: %u\n", (reg_val0 >> 3) & 0x1);
	return len;
}
#endif
...
#ifdef AUTOX			
#ifdef QDMA_TEST
static DEVICE_ATTR(test_case, S_IWUSR | S_IRUGO,
                       show_libqdma_cash3_test_cases, run_libqdma_cash3_test_case);
#endif
static DEVICE_ATTR(host_cmd, S_IWUSR | S_IRUGO,
                      show_host_cmd, trigger_host_cmd);
static DEVICE_ATTR(video_sts, S_IWUSR | S_IRUGO,
			show_video_sts, NULL);
static DEVICE_ATTR(fw_ver, S_IWUSR | S_IRUGO,
			show_fw_ver, NULL);
static DEVICE_ATTR(hw_status, S_IWUSR | S_IRUGO,
			show_hw_status, NULL);
#endif			
...
static struct attribute *pci_device_attrs1[] = {
		&dev_attr_qmax.attr,
		&dev_attr_intr_rngsz.attr,
#ifdef AUTOX		
		&dev_attr_host_cmd.attr,
		&dev_attr_video_sts.attr,
		&dev_attr_fw_ver.attr,
		&dev_attr_hw_status.attr,
#ifdef QDMA_TEST
		&dev_attr_test_case.attr,
#endif		
#endif		
		NULL,
};

static struct attribute *pci_master_device_attrs1[] = {
		&dev_attr_qmax.attr,
		&dev_attr_intr_rngsz.attr,
#ifndef __QDMA_VF__
...
// add qdma test cases
static struct attribute_group pci_device_attr_group1 = {
		.name  = "qdma",
		.attrs = pci_device_attrs1,
};

static struct attribute_group pci_master_device_attr_group1 = {
		.name  = "qdma",
		.attrs = pci_master_device_attrs1,
};
...
#ifdef AUTOX
	mutex_init(&xpdev->cash3_cmd_interrupt_lock);
	xpdev->cash3_shadow_fpga_irq_status = 0;
#endif
	return xpdev;
...
static int probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (conf.master_pf) {
		if (qdma_test)
			rv = sysfs_create_group(&pdev->dev.kobj, &pci_master_device_attr_group1);
		else
		...
		if (qdma_test)
			rv = sysfs_create_group(&pdev->dev.kobj, &pci_device_attr_group1);
		else
	...
	read_io_area_conf(xpdev->dev_hndl, &xpdev->ior_addr, &xpdev->ior_size);
...
#define TEST
	...
	{
		uint32_t value = 0;
		rv = qdma_device_read_user_register(xpdev, 0x0, &value);
		if (rv < 0)
			goto close_device;
	...
	if (!autox_sysm_alloc(xpdev)) {
		(void) autox_sysm_init(xpdev);
	}

#ifdef AUTOX_ETC
	if (!autox_gps_alloc(xpdev)) {
		(void) autox_gps_init(xpdev);
	}
#endif

	if (!autox_video_alloc(xpdev)) {
		(void) autox_video_init_all(xpdev);
	}

	return write_cash3_user_reg(dev_hndl, CASH3_HW_IRQ_MASK0_ADDR, 0xffffffff) 
		| write_cash3_user_reg(dev_hndl, CASH3_HW_IRQ_MASK1_ADDR, 0xffffffff) ;
#else
	return 0;
#endif
...
static void remove_one(struct pci_dev *pdev)
{
...
#ifdef AUTOX
	autox_video_finish_all(xpdev);
	autox_video_free(xpdev);

#ifdef AUTOX_ETC
	autox_gps_finish(xpdev);
	autox_gps_free(xpdev);
#endif

	autox_sysm_finish(xpdev);
	autox_sysm_free(xpdev);
#endif

	if (xdev->conf.master_pf)
	{
		if (qdma_test)
			sysfs_remove_group(&pdev->dev.kobj, &pci_master_device_attr_group1);
		else
	...
	{
		if (qdma_test)
			sysfs_remove_group(&pdev->dev.kobj, &pci_device_attr_group1);
		else
...
#ifdef AUTOX
static int __init acash_mod_init(void)
#else
...
#endif
{
	...
	pr_info("%s", version);
	pr_info("num_threads=%d\n", num_threads);

	pr_info("option to enable qdma test cases = %d\n", qdma_test);
...
#ifdef AUTOX
static void __exit acash_mod_exit(void)
#else
...
#ifdef AUTOX
module_init(acash_mod_init);
module_exit(acash_mod_exit);
#else
