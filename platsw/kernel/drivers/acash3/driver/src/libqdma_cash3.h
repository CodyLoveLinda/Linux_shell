...
#ifndef __LIBQDMA_CASH3_H__
#define __LIBQDMA_CASH3_H__
...
/******************************************************************************/
/**
 * cash3_qdma_get_handles() -  get devices and queue handles for existing qdma
 * configuration
 ...
 * @host_cmd_qhndl: cmd queue handle ptr holding handle result - can be NULL
 * @fpga_event_qhndl: event queue handle ptr holding handle result - can be NULL
 * @return	0 on success
...
/******************************************************************************/
/**
 * initialize_cash3_video() -  call this one time at the beginning to prepare
...
/******************************************************************************/
/**
 * clear_cash3_video_status() -  resync host with FPGA
 ...
 * @return	on failure with Linux error code
 *****************************************************************************/
int clear_cash3_video_status(unsigned long dev_hndl, uint8_t video_stream_id
...
/******************************************************************************/
/**
 * @struct - cash3_qdma_video_request
...
struct cash3_qdma_video_request {
	...
	struct pci_dev *pdev;
	size_t buffer_chunk_count;              	//how many buffer chunks, read only
	size_t buffer_chunk_size;               	//each chunk's byte size, read only
...
/******************************************************************************/
/**
 * prepare_cash3_qdma_video_request() - prepare a request for video read use based 
 ...
 * Only prepared cash3_qdma_video_request can be used in 
 * send_cash3_qdma_video_request. call it multiple times on the same req
...
/******************************************************************************/
/**
 * unprepare_cash3_qdma_video_request() - release the resources obtained by 
 * prepare_video_request including buffers
 *
 * @cash3_qdma_request: the request that needs to get teared down
 *****************************************************************************/
...
/******************************************************************************/
/**
 * send_cash3_qdma_video_request() - send out a cash3 qdma video read request
 *
 * @dev_hndl: qdma device handle
 * @qhndl: qdma queue handle
 * @vreq: pointer to a cash3_qdma_video_request which need to be prepared using 
 * 			prepare_cash3_qdma_video_request	
...
/******************************************************************************/
/**
 * prepare_cash3_qdma_request() - prepare a request for data write or read use based 
 * on its data members, only need to call this once if no data member changes.
 * The IO buffer chunks are allocated in this function, the user needs to specify
 * the size and number of chunks
 * Only prepared cash3_qdma_request can be used in 
 * send_cash3_qdma_request
 *
...
/******************************************************************************/
/**
 * unprepare_cash3_qdma_request() - release the resources obtained by 
 * prepare_video_request including buffers
 *
...
/******************************************************************************/
/**
 * send_cash3_qdma_request() - send out a cash3 qdma write/read request
 *
 * @qreq: pointer to a cash3_qdma_request which need to be prepared using 
 * 			prepare_cash3_qdma_request	
 * @return	0 on non-blocking submission success, IO byte count on blocking
 * @return	<0 on failure with negated Linux error code
 *****************************************************************************/
 ...
 * @dev_hndl: qdma device handle
 * @cmd: pointer to a cmd struct - see cash3_host_cmds.h
 * @cmd_size: the size of the above cmd
 * @wait_until_done: block the call until they are finished by FPGA 
 * and the caller can check the CASH3_SCRATCH_REG_CMD_STATUS_ADDR[n] for the
 * n'th sub cmd status
 ...
 * @return	<0 on failure with negated Linux error code
 *****************************************************************************/
// ssize_t get_fpga_event(unsigned long dev_hndl, struct cash3_fpga_event_t* event);
